...
main(int argc, char** argv)
{
    ...
    while (running) {
        ...
        float trTemp = IedServer_getFloatAttributeValue(iedServer, IEDMODEL_GenericIO_GGIO1_NamPlt_IED20T);

        if (trTemp>60.0 && timestamp > gooseInterval + lastSentTime)
        {
            if (timestamp > gooseInterval + lastSentTime)
            {   
                // publishGooseMsg(getReceiverId("SERVER_IED7"), 10, MmsValue_newBoolean(false));
                // publishGooseMsg(getReceiverId("SERVER_IED1"), 2, MmsValue_newBoolean(false));
                // publishGooseMsg(getReceiverId("SERVER_IED16"), 2, MmsValue_newBoolean(false));
                publishGooseMsg("SERVER_IED7", 10, MmsValue_newBoolean(false));
                publishGooseMsg("SERVER_IED1", 2, MmsValue_newBoolean(false));
                publishGooseMsg("SERVER_IED16", 2, MmsValue_newBoolean(false));
