...
void accessReadings()
{
    bool closedInCircuit = IedStatus[0] && IedStatus[1] && IedStatus[2];
    if (closedInCircuit)
    {
        MmsValue *inputCurrent = MmsValue_newFloat(1000.0);
        IedServer_updateAttributeValue(iedServer, IEDMODEL_GenericIO_GGIO1_NamPlt_IED1I, inputCurrent);
    }
    else
    {
        MmsValue *inputCurrent = MmsValue_newFloat(0.0);
        IedServer_updateAttributeValue(iedServer, IEDMODEL_GenericIO_GGIO1_NamPlt_IED1I, inputCurrent);
    }
...
{
    writeLog("Caught on: controlHandlerForBinaryOutput handle\n");
    if (test)
        return CONTROL_RESULT_FAILED;

    if (MmsValue_getType(value) == MMS_BOOLEAN)
    {
        writeLog("received binary control command: ");
    ...
    {
        IedServer_updateAttributeValue(iedServer, IEDMODEL_GenericIO_GGIO1_NamPlt_IED1S, value);
        IedStatus[0] = MmsValue_getBoolean(value);
        accessReadings();
        // publishGooseMsg(26, 0, value);
        // publishGooseMsg("SERVER_IED16", 0, value);
        publishGooseMsg("SERVER_IED16", 0, value);
...
int main(int argc, char **argv)
{
    ...
    while (running)
    {
        ...
        float inputVoltage = IedServer_getFloatAttributeValue(iedServer, IEDMODEL_GenericIO_GGIO1_NamPlt_IED1V);

        if (inputVoltage > 33000.0 && timestamp > gooseInterval + lastSentTime)
        {
            IedServer_updateTimestampAttributeValue(iedServer, IEDMODEL_GenericIO_GGIO1_NamPlt_IED1S, &iecTimestamp);
            IedServer_updateBooleanAttributeValue(iedServer, IEDMODEL_GenericIO_GGIO1_NamPlt_IED1S, false);
            IedStatus[0] = false;
            accessReadings();

            if (timestamp > gooseInterval + lastSentTime)
            {
                // publishGooseMsg(26, 0, MmsValue_newBoolean(false));
                publishGooseMsg("SERVER_IED16", 0, MmsValue_newBoolean(false));
                lastSentTime = Hal_getTimeInMs();
            }
        }
